...
namespace Hyperf\Zby\Office\Excel;


use Hyperf\Context\ApplicationContext;
use Hyperf\Zby\Exception\ZbyException;
use Hyperf\Zby\Office\ZbyExcel;
...
class PhpOffice extends ZbyExcel
{
    public function import(ZbyModel $model, ?\Closure $closure = null): bool
    {

        $request = ApplicationContext::getContainer()->get(ZbyRequest::class);
        $data = [];
        if ($request->hasFile('file')) {
            ...
            try {
                ...
                unlink($tempFilePath);
                throw new ZbyException($e->getMessage());
            }
        } else {
            return false;
        }
